...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("passwordcoin-miner");
...
{
    LogPrintf("passwordcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("passwordcoinMiner-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running passwordcoinMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("passwordcoinMiner terminated\n");
        throw;
    }
}
